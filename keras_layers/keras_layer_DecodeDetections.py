...
class DecodeDetections(Layer):
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            coords (str, optional): The box coordinate format that the model outputs. Must be 'centroids' i.e. the
                format `(cx, cy, w, h)` (box center coordinates, width, and height). Other coordinate formats are
                currently not supported.
            normalize_coords (bool, optional): Set to `True` if the model outputs relative coordinates (i.e. coordinates
                in [0,1]) and you wish to transform these relative coordinates back to absolute coordinates.
                If the model outputs relative coordinates, but you do not want to convert them back to absolute
                coordinates, set this to `False`.
    ...
    def call(self, y_pred, mask=None):
        ...
        Args:
            y_pred: shape 为 (batch_size, total_num_boxes, num_classes + 12)
            mask:
        Returns:
            3D tensor of shape `(batch_size, top_k, 6)`. The second axis is zero-padded to always yield `top_k`
            predictions per batch item. The last axis contains the coordinates for each predicted box in the format
        ...
        def filter_predictions(batch_item):
            ...
            def filter_single_class(index):
                # From a tensor of shape (n_boxes, n_classes + 4) extract a tensor of shape (n_boxes, 1 + 1 + 4) that
                # contains the confidence values for just one class, determined by `index`.
                ...
                def perform_nms():
                    ...
                    # Note `tf.image.non_max_suppression()` needs the box coordinates in the format
                    #  `(ymin, xmin, ymax, xmax)`.
                    xmin_ = tf.expand_dims(single_class[..., -4], axis=-1)
                    ymin_ = tf.expand_dims(single_class[..., -3], axis=-1)
                    xmax_ = tf.expand_dims(single_class[..., -2], axis=-1)
                    ymax_ = tf.expand_dims(single_class[..., -1], axis=-1)
                    boxes = tf.concat(values=[ymin_, xmin_, ymax_, xmax_], axis=-1)
            ...
            # Perform top-k filtering for this batch item or pad it in case there are fewer than `self.top_k` boxes
            # left at this point. Either way, produce a tensor of length `self.top_k`.
            # By the time we return the final results tensor for the whole batch, all batch items must have the same
            # number of predicted boxes so that the tensor dimensions are homogenous. If fewer than `self.top_k`
            # predictions are left after the filtering process above, we pad the missing predictions with zeros as dummy
            # entries.
            def top_k():
            ...
            top_k_boxes = tf.cond(tf.greater_equal(tf.shape(filtered_predictions)[0], self.tf_top_k),
                                  top_k,
                                  pad_and_top_k)
            return top_k_boxes
