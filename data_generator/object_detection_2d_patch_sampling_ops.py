...
class PatchCoordinateGenerator:
    ...
    def __init__(self,
        ...
        """
        Arguments:
            img_height (int): The height of the image for which the patch coordinates shall be generated. Doesn't have
                to be known upon construction.
            img_width (int): The width of the image for which the patch coordinates shall be generated. Doesn't have to
                be known upon construction.
            must_match (str, optional): Can be either of 'h_w', 'h_ar', and 'w_ar'.
                Specifies which two of the three quantities height, width, and aspect ratio determine the shape of the
                generated patch. The respective third quantity will be computed from the other two.
                For example, if `must_match == 'h_w'`, then the patch's height and width will be set to lie within
                [min_scale, max_scale] of the image size or to `patch_height` and/or `patch_width`, if given.
                The patch's aspect ratio is the dependent variable in this case, it will be computed from the height and
                width. Any given values for `patch_aspect_ratio`, `min_aspect_ratio`, or `max_aspect_ratio` will be
                ignored.
            min_scale (float, optional): The minimum size of a dimension of the patch as a fraction of the respective
                dimension of the image. Can be greater than 1.
                For example, if the image width is 200 and `min_scale == 0.5`, then the width of the generated patch
                will be at least 100. If `min_scale == 1.5`, the width of the generated patch will be at least 300.
            max_scale (float, optional): The maximum size of a dimension of the patch as a fraction of the respective
                dimension of the image. Can be greater than 1.
                For example, if the image width is 200 and `max_scale == 1.0`, then the width of the generated patch
                will be at most 200. If `max_scale == 1.5`, the width of the generated patch will be at most 300.
                Must be greater than `min_scale`.
            scale_uniformly (bool, optional): If `True` and if `must_match == 'h_w'`, the patch height and width will be
                scaled uniformly, otherwise they will be scaled independently.
            min_aspect_ratio (float, optional): Determines the minimum aspect ratio for the generated patches.
            max_aspect_ratio (float, optional): Determines the maximum aspect ratio for the generated patches.
            patch_ymin (int, optional): `None` or the vertical coordinate of the top left corner of the generated
                patches. Note: patch_ymin 和 patch_xmin 都是以 image 的左上角的点作为坐标原点
                If this is not `None`, the position of the patches along the vertical axis is fixed.
                Note if this is `None`, then the vertical position of generated patches will be chosen randomly such
                 that the overlap of a patch and the image along the vertical dimension is always maximal.
                 就是说让这个 patch 和 image 重合的部分最大
            patch_xmin (int, optional): `None` or the horizontal coordinate of the top left corner of the generated
                patches.
                If this is not `None`, the position of the patches along the horizontal axis is fixed.
                If this is `None`, then the horizontal position of generated patches will be chosen randomly such that
                the overlap of a patch and the image along the horizontal dimension is always maximal.
            ...
            patch_aspect_ratio (float, optional): `None` or the fixed aspect ratio of the generated patches.
        """

        if must_match not in {'h_w', 'h_ar', 'w_ar'}:
            ...
            if scale_uniformly:
                if not (isinstance(min_scale, float) and isinstance(max_scale, float)):
                    raise ValueError("`min_scale` and `max_scale` must be float"
                                     "when `must_match`=='h_w' and scale_uniformly is true.")
                elif min_scale >= max_scale:
                    raise ValueError("It must be `min_scale < max_scale`.")
            else:
                # patch_height, patch_width 如果有一个不是 int, 那么就需要 min_scale 和 max_scale 来把它算出来
                if (not isinstance(patch_height, int)) or (not isinstance(patch_width, int)):
                    if not (isinstance(min_scale, float) and isinstance(max_scale, float)):
                        raise ValueError("`min_scale` and `max_scale` must be float"
                                         "when `patch_height` or `patch_width` is not int")
                    elif min_scale >= max_scale:
                        raise ValueError("It must be `min_scale < max_scale`.")
        elif must_match == 'h_ar':
            if not isinstance(patch_height, int):
                if not (isinstance(min_scale, float) and isinstance(max_scale, float)):
                    raise ValueError("`min_scale` and `max_scale` must be float"
                                     "when `patch_height` is not int and `must_match` == 'h_ar'")
                elif min_scale >= max_scale:
                    raise ValueError("It must be `min_scale < max_scale`.")
            elif not isinstance(patch_aspect_ratio, float):
                if not (isinstance(min_aspect_ratio, float) and isinstance(max_aspect_ratio, float)):
                    raise ValueError("`min_aspect_ratio` and `max_aspect_ratio` must be float"
                                     "when `patch_aspect_ratio` is not float and `must_match` == 'h_ar'")
                elif min_aspect_ratio >= max_aspect_ratio:
                    raise ValueError("It must be `min_aspect_ratio < max_aspect_ratio`.")
        # must_match == 'w_ar'
        else:
            if not isinstance(patch_width, int):
                if not (isinstance(min_scale, float) and isinstance(max_scale, float)):
                    raise ValueError("`min_scale` and `max_scale` must be float"
                                     "when `patch_width` is not int and `must_match` == 'w_ar'")
                elif min_scale >= max_scale:
                    raise ValueError("It must be `min_scale < max_scale`.")
            elif not isinstance(patch_aspect_ratio, float):
                if not (isinstance(min_aspect_ratio, float) and isinstance(max_aspect_ratio, float)):
                    raise ValueError("`min_aspect_ratio` and `max_aspect_ratio` must be float"
                                     "when `patch_aspect_ratio` is not float and `must_match` == 'w_ar'")
                elif min_aspect_ratio >= max_aspect_ratio:
                    raise ValueError("It must be `min_aspect_ratio < max_aspect_ratio`.")
    ...
    def __call__(self):
        """
        Returns:
            A 4-tuple `(ymin, xmin, height, width)` that represents the coordinates of the generated patch.
        """
        ...
        # Width is the dependent variable.
            ...
            if not isinstance(self.patch_aspect_ratio, float):
...
class CropPad:
    ...
    Depending on the given output patch size and the position (top left corner) relative to the input image, the image
    will be cropped and/or padded along one or both spatial dimensions.

    For example, if the output patch lies entirely within the input image, this will result in a regular crop.
    If the input image lies entirely within the output patch, this will result in the image being padded in every
    direction.
    All other cases are mixed cases where the image might be cropped in some directions and padded in others.

    The output patch can be arbitrary in both size and position as long as it overlaps with the input image.
    """
    ...
    def __init__(self,
        ...
        """
        Arguments:
            patch_ymin (int, optional): The vertical coordinate of the top left corner of the output patch relative to
                the image coordinate system. Can be negative (i.e. lie outside the image) as long as the resulting patch
                still overlaps with the image.
            patch_xmin (int, optional): The horizontal coordinate of the top left corner of the output patch relative to
                the image coordinate system. Can be negative (i.e. lie outside the image) as long as the resulting patch
                still overlaps with the image.
            patch_height (int, optional): The height of the patch to be sampled from the image. Can be greater than the
                height of the input image.
            patch_width (int, optional): The width of the patch to be sampled from the image. Can be greater than the
            ...
            box_filter (BoxFilter, optional): Only relevant if ground truth bounding boxes are given.
                ...
                transformation. Refer to the `BoxFilter` documentation for details.
                If `None`, the validity of the bounding boxes is not checked.
            background (list/tuple, optional): A 3-tuple specifying the RGB color value of the potential background
                pixels of the scaled images. In the case of single-channel images, the first element of `background`
                will be used as the background pixel value.
            labels_format (list/tuple, optional): A list/tuple that defines which index in the last axis of the labels
                of an image. The list/tuple contains at least the keywords 'xmin', 'ymin', 'xmax', and 'ymax'.
        """
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        ...
        if (self.patch_ymin + self.patch_height < 0) or (self.patch_xmin + self.patch_width < 0):
        ...
        if image.ndim == 3:
            ...
            raise ValueError('`image` is not valid, because its ndim is {}'.format(image.ndim))

        # Perform the crop.
        if patch_ymin < 0:
            # Pad the image at the top and on the left.
            if patch_xmin < 0:
                ...
                canvas[-patch_ymin:-patch_ymin + image_crop_height, -patch_xmin:-patch_xmin + image_crop_width] = \
                    image[:image_crop_height, :image_crop_width]
            # Pad the image at the top and crop it on the left.
            else:
                # The number of pixels of the image that will end up on the canvas in the vertical direction.
                image_crop_height = min(img_height, self.patch_height + patch_ymin)
                # The number of pixels of the image that will end up on the canvas in the horizontal direction.
                image_crop_width = min(self.patch_width, img_width - patch_xmin)
                canvas[-patch_ymin:-patch_ymin + image_crop_height, :image_crop_width] = \
                    image[:image_crop_height, patch_xmin:patch_xmin + image_crop_width]
        else:
            # Crop the image at the top and pad it on the left.
            if patch_xmin < 0:
                # The number of pixels of the image that will end up on the canvas in the vertical direction.
                image_crop_height = min(self.patch_height, img_height - patch_ymin)
                # The number of pixels of the image that will end up on the canvas in the horizontal direction.
                image_crop_width = min(img_width, self.patch_width + patch_xmin)
                canvas[:image_crop_height, -patch_xmin:-patch_xmin + image_crop_width] = \
                    image[patch_ymin:patch_ymin + image_crop_height, :image_crop_width]
            # Crop the image at the top and on the left.
            else:
                # The number of pixels of the image that will end up on the canvas in the vertical direction.
                image_crop_height = min(self.patch_height, img_height - patch_ymin)
                # The number of pixels of the image that will end up on the canvas in the horizontal direction.
                image_crop_width = min(self.patch_width, img_width - patch_xmin)
                canvas[:image_crop_height, :image_crop_width] = \
                    image[patch_ymin:patch_ymin + image_crop_height, patch_xmin:patch_xmin + image_crop_width]
        image = canvas
        if return_inverter:
            def inverter(translated_labels):
                inverted_labels = np.copy(translated_labels)
                # Adam why + 1?
                # labels[:, [ymin + 1, ymax + 1]] += patch_ymin
                # labels[:, [xmin + 1, xmax + 1]] += patch_xmin
                inverted_labels[:, [ymin, ymax]] += patch_ymin
                inverted_labels[:, [xmin, xmax]] += patch_xmin
...
class RandomPatch:
    """
    Randomly samples a patch from an image. The randomness refers to whatever randomness may be introduced by the patch
    coordinate generator, the box filter, and the image validator.

    Input images may be cropped and/or padded along either or both of the two spatial dimensions as necessary in order
    to obtain the required patch.

    As opposed to `RandomPatchInf`, it is possible for this transform to fail to produce an output image at all, in
    which case it will return `None`. This is useful, because if this transform is used to generate patches of a fixed
    size or aspect ratio, then the caller needs to be able to rely on the output image satisfying the set size or aspect
    ratio. It might therefore not be an option to return the unaltered input image as other random transforms do when
    they fail to produce a valid transformed image.
    """
    ...
    def __init__(self,
        ...
        """
        Arguments:
            patch_coord_generator (PatchCoordinateGenerator): A `PatchCoordinateGenerator` object to generate the
                positions and sizes of the patches to be sampled from the input images.
            box_filter (BoxFilter, optional): Only relevant if ground truth bounding boxes are given.
                A `BoxFilter` object to filter out bounding boxes that don't meet the given criteria after the
                transformation. Refer to the `BoxFilter` documentation for details.
                If `None`, the validity of the bounding boxes is not checked.
            image_validator (ImageValidator, optional): Only relevant if ground truth bounding boxes are given.
                An `ImageValidator` object to determine whether a sampled patch is valid.
                If `None`, any outcome is valid.
            n_trials_max (int, optional): Only relevant if ground truth bounding boxes are given.
                Determines the maxmial number of trials to sample a valid patch. If no valid patch could be sampled in
                `n_trials_max` trials, returns one `None` in place of each regular output.
            clip_boxes (bool, optional): Only relevant if ground truth bounding boxes are given.
                If `True`, any ground truth bounding boxes will be clipped to lie entirely within the sampled patch.
        ...
        if p < self.prob:
            ...
            if self.can_fail:
                if return_inverter:
                    ...
                    return None, labels, inverter
                else:
                    return None, labels
            # If we weren't able to sample a valid patch, return the unaltered input image.
            else:
                if return_inverter:
                    def inverter(translated_labels):
                        return translated_labels
...
class RandomPatchInf:
    """
    Randomly samples a patch from an image. The randomness refers to whatever randomness may be introduced by the patch
    coordinate generator, the box filter, and the patch validator.

    Input images may be cropped and/or padded along either or both of the two spatial dimensions as necessary in order
    to obtain the required patch.

    This operation is very similar to `RandomPatch`, except that:
    1. This operation runs indefinitely until either a valid patch is found or the input image is returned unaltered,
        i.e. it cannot fail.
    ...
    def __init__(self,
        ...
        Arguments:
            ...
            image_validator (ImageValidator, optional): Only relevant if ground truth bounding boxes are given.
                An `ImageValidator` object to determine whether a sampled patch is valid.
                If `None`, any outcome is valid.
            bound_generator (BoundGenerator, optional): A `BoundGenerator` object to generate upper and lower bound
                values for the patch validator. Every `n_trials_max` trials, a new pair of upper and lower bounds will
                be generated until a valid patch is found or the original image is returned. This bound generator
                overrides the bound generator of the patch validator.
            n_trials_max (int, optional): Only relevant if ground truth bounding boxes are given.
                The sampler will run indefinitely until either a valid patch is found or the original image is returned,
                but this determines the maxmial number of trials to sample a valid patch for each selected pair of lower
                and upper bounds before a new pair is picked.
            clip_boxes (bool, optional): Only relevant if ground truth bounding boxes are given.
                If `True`, any ground truth bounding boxes will be clipped to lie entirely within the sampled patch.
            prob (float, optional): `(1 - prob)` determines the probability with which the original, unaltered image is
                returned.
            background (list/tuple, optional): A 3-tuple specifying the RGB color value of the potential background
                ...
                will be used as the background pixel value.
            labels_format (list/tuple, optional): A list/tuple that defines which index in the last axis of the labels
                of an image. The list/tuple maps at least the keywords 'xmin', 'ymin', 'xmax', and 'ymax'.
        """
    ...
    def __call__(self, image, labels=None, return_inverter=False):
        ...
        # Keep going until we either find a valid patch or return the original image.
        while True:
            p = np.random.uniform(0, 1)
            if p < self.prob:
                ...
                # Use at most `self.n_trials_max` attempts to find a crop that meets our requirements.
                for _ in range(max(1, self.n_trials_max)):
                    # Generate patch coordinates.
                    patch_ymin, patch_xmin, patch_height, patch_width = self.patch_coord_generator()
                    self.sample_patch.patch_ymin = patch_ymin
                    ...
                    if not (self.patch_coord_generator.min_aspect_ratio <= aspect_ratio
