...
class SSDRandomCrop:
    """
    Performs the same random crops as defined by the `batch_sampler` instructions of the original Caffe implementation
    of SSD. A description of this random cropping strategy can also be found in the data augmentation section of the
    ...
    def __init__(self,
        ...
        # This randomly samples one of the lower IoU bounds defined by the `sample_space` every time it is called.
        ...
        # Produces coordinates for candidate patches such that the height and width of the patches are between 0.3 and
        # 1.0 of the height and width of the respective image and the aspect ratio of the patches is between 0.5 and
        # 2.0.
        ...
        # Filters out boxes whose center point does not lie within the chosen patches.
        ...
        # Defines a patch to be valid if at least one ground truth bounding box (n_boxes_min == 1) has an IoU overlap
        # with the patch that meets the requirements defined by `bound_generator`.
        ...
        # Runs until either a valid patch is found or the original input image is returned unaltered.
        # Runs a maximum of 50 trials to find a valid patch for each new sampled IoU threshold.
        # Every 50 trials, the original image is returned as is with probability (1 - prob) = 0.143.
...
class SSDExpand:
    """
    Performs the random image expansion as defined by the `train_transform_param` instructions of the original Caffe
    implementation of SSD. A description of this expansion strategy can also be found in section 3.6
    ("Data Augmentation for Small Object Accuracy") of the paper: https://arxiv.org/abs/1512.02325
    """
    ...
    def __init__(self,
        ...
        self.labels_format = labels_format
        # Generate coordinates for patches that are between 1.0 and 4.0 times the size of the input image in both
        # spatial dimensions.
        self.patch_coord_generator = PatchCoordinateGenerator(must_match='h_w',
                                                              min_scale=1.0,
                                                              max_scale=4.0,
        ...
        # With probability 0.5, place the input image randomly on a canvas filled with mean color values according to
        # the parameters set above. With probability 0.5, return the input image unaltered.
...
class SSDPhotometricDistortions:
    """
    Performs the photometric distortions defined by the `train_transform_param` instructions of the original Caffe
...
class SSDDataAugmentation:
    """
    Reproduces the data augmentation pipeline used in the training of the original Caffe implementation of SSD.
    """
