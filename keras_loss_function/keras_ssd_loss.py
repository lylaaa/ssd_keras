...
class SSDLoss:
    ...
    def __init__(self,
        ...
        Arguments:
            neg_pos_ratio (int, optional): The maximum ratio of negative (i.e. background)
                ...
                but `y_true` contains anchor boxes labeled with the background class.
                Since the number of background boxes in `y_true` will usually exceed
            ...
            n_neg_min (int, optional): The minimum number of negative ground truth boxes to
                enter the loss computation *per batch*.
                This argument can be used to make sure that
                the model learns from a minimum number of negatives in batches in which there are very few,
                or even none at all, positive ground truth boxes.
                It defaults to 0 and if used, it should be set to a value that
    ...
    def smooth_L1_loss(self, y_true, y_pred):
        ...
        Returns:
            The smooth L1 loss, a nD-1 Tensorflow tensor.
            In this context a 2D tensor of shape (batch, n_boxes_total).
            n_boxes_total 为所有 feature_map 上的 box 的总个数
    ...
    def log_loss(self, y_true, y_pred):
        ...
        Returns:
            The softmax log loss, a nD-1 Tensorflow tensor.
            In this context a 2D tensor of shape (batch, n_boxes_total).
        """
    ...
    def compute_loss(self, y_true, y_pred):
        ...
        Arguments:
            y_true (array): A Numpy array of shape `(batch_size, #boxes, #classes + 12)`,
                ...
                Be careful to make sure that the index of each given box in `y_true` is the same as the index for the
                corresponding box in `y_pred`. The last axis must have length `#classes + 12` and contain
                `[classes one-hot encoded, 4 ground truth box coordinate offsets, 8 arbitrary entries]`
                in this order, including the background class.
                The last eight entries of the last axis are not used by this function and therefore their contents are
                irrelevant, they only exist so that `y_true` has the same shape as `y_pred`,
                where the last four entries of the last axis contain the anchor box coordinates,
                which are needed during inference.
                Important: Boxes that you want the cost function to ignore need to have a one-hot class vector of all
                zeros.
        ...
        # Output dtype: tf.int32
        # Note that `n_boxes` in this context denotes the total number of boxes per image,
        ...
        # In the unlikely case when either :
        #   (1) there are no negative ground truth boxes at all
        ...
        def f2():
            ...
            # Tensor of shape (batch_size * n_boxes,)
            # tf.scatter_nd 的功能是新建一个初始化为 0 的数组, 然后把 updates 的内容分散到该数组对应 indices 的位置上
        ...
        # In case `n_positive == 0`, use tf.maximum(1.0, n_positive)
        # UNCLEAR: 既然 neg_class_loss 也参与了计算, 不知道为什么也是除以 n_positive, 而不是 n_positive + n_negative_keep
        ...
        # 意思是 keras 之后会把 total_loss 除以 batch_size, 所以先乘一下
